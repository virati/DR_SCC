...
for pt in all_pts:
    ...
    all_obs = [
        (
            ...
            cc["Date"],
            cc["Phase"],
            ClinFrame.clin_dict["DBS" + pt][cc["Phase"]]["HDRS17"],
        )
        for cc in analysis.YFrame.file_meta
        if cc["Patient"] == pt and cc["Circadian"] == "night"
    ]
    ...
    for phph, phase in enumerate(Phase_List("ephys")):
        all_state = np.array(
            [
                [
                    (
                ...
                for (pp, cc, dd, ee) in all_obs
                if dd == phase
            ]
        )  # .reshape(-1,10,order='C')
        avg_state[phph, :, :] = np.median(all_state, axis=0)
        avg_traj[phph] = (
            np.dot(avg_state[phph, 0, :], lridge),
            np.dot(avg_state[phph, 1, :], rridge),
        )
