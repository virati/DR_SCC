...
hdrs_aucs = np.array(
    [[(r["HDRS"][0], r["HDRS"][1]) for r in run] for run in auc_curves_from_run]
).reshape(-1, 2)
cb_aucs = np.array(
    [[(r["CB"][0], r["CB"][1]) for r in run] for run in auc_curves_from_run]
).reshape(-1, 2)
rand_aucs = np.array(
    [[(r["Random"][0], r["Random"][1]) for r in run] for run in auc_curves_from_run]
).reshape(-1, 2)
...
for rr in range(8):
    for itr in range(5):
        for algo in algo_list:
            hdrs_func = interp1d(
                auc_curves_from_run[rr][itr][algo][1],
                auc_curves_from_run[rr][itr][algo][0],
                kind="zero",
            )
...
for algo in algo_list:
    ...
    plt.fill_between(
        mean_blah, auc_lower, auc_upper, color=color_code[algo], alpha=0.05
    )
...
# stack nulls
plt.figure()
all_nulls = np.array(null_curves_from_run).reshape(
    -1,
)
...
for aa, algo in enumerate(algo_list[0:end_algo]):
    plt.hist(
        algo_aucs[:, aa],
        stacked=False,
        color=color_code[algo],
        bins=bins,
        label=algo,
        alpha=0.5,
    )
    plt.vlines(
        np.median(algo_aucs[:, aa]),
        0,
        line_height + 10 * aa,
        color=color_code[algo],
        label=algo,
    ...
    plt.hlines(
        line_height + 10 * aa,
        np.median(algo_aucs[:, aa]) - algo_std,
        np.median(algo_aucs[:, aa]) + algo_std,
        color=color_code[algo],
        linewidth=5,
    )
...
plt.vlines(
    np.median(all_nulls), 0, line_height + 20, color="green", label="Null", linewidth=5
)
